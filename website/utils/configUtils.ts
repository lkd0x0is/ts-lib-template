...
function isValidPackageDir(entry: fs.Dirent): boolean {
  ...
  return entry.isDirectory() && !packageName.startsWith(".") && packageName !== "template";
}
...
export function getPackagesWithApiDocs(packagesBasePath: string): string[] {
  ...
  try {
    const packages = fs.readdirSync(packagesBasePath, { withFileTypes: true })
      .filter(isValidPackageDir)
      .map((entry) => entry.name)
      .filter((packageName) => {
        const apiDocsPath = path.join(packagesBasePath, packageName, "docs-markdown");
        const hasDocs = fs.existsSync(apiDocsPath) &&
    ...
    if (packages.length === 0) {
      ...
      logger.info(`Successfully found ${packages.length} package(s) with API docs.`);
    }
    return packages;

  } catch (error) {
    logger.error(`Error while scanning packages directory ${packagesBasePath}:`, error);
    return [];
  }
}
...
export function generateApiDocsPlugins(
  ...
  return packages.map((packageName) => {
    const apiDocsPath = path.join("..", "packages", packageName, "docs-markdown");
